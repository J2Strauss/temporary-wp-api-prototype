        ...
        <div class="postbox">
            <div style="float: left; width: 98%; clear: both;" class="inside">
                <table cellpadding="5" cellspacing="5" width="100%">
                    <tr>
                        <td width="150">
                            <legend for="instance_name" style="display:inline;"><?php _e('Name','event-clndr'); ?></legend> <span style="color:red;">*</span>
                        </td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="instance_template" style="display:inline;"><?php _e('HTML Template','event-clndr'); ?></legend><br />
                            <br />
                            <strong><?php _e("Reference","event-clndr"); ?></strong><br />
                            <a href="https://github.com/kylestetz/CLNDR#introduction-you-write-the-markup" target="_blank"><?php _e("CLNDR.js Markup","event-clndr"); ?></a><br />
                            <a href="http://momentjs.com/docs/#/parsing/string-format/" target="_blank"><?php _e("Moment.js Formatting","event-clndr"); ?></a>
                        </td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="instance_options" style="display:inline;"><?php _e('JS Object Options','event-clndr'); ?></legend><br />
                            <span style="display:block;padding-top:10px;font-size:0.9em;line-height:1.2;d">
                                (<?php _e('No need to include event and template data, this is automatically populated','event-clndr'); ?>)<br />
                            </span>
                            <br />
                            <strong><?php _e("Reference","event-clndr"); ?></strong><br />
                            <a href="https://github.com/kylestetz/CLNDR#usage" target="_blank"><?php _e("CLNDR.js Options","event-clndr"); ?></a><br />
                            <br />
                            <strong><?php _e("Disclaimer","event-clndr"); ?></strong><br />
                            <span style="font-size:0.9em;line-height:1.2;">
                                <?php _e("It is recommended to only add Javascipt options here if you are a web developer, or generally know what you are doing. Incorrectly formatted input could hinder other Javascript functionality on your Wordpress website.","event-clndr"); ?>
                            </span>
                        </td>
...
function admin_CLNDR_INSTANCES_display_instances(){
    ...
    if ( !empty($instances) ) {
    ?>
        <p><?php _e("CLNDR shortcodes can be added into any page or post, as well as into the content of a 'Text' widget for sidebar or footer use.","event-clndr"); ?><p>

        <?php // if admin etc {

            $styles_url = get_bloginfo('wpurl') . "/wp-admin/admin.php?page=clndr-styles";
            $styles_text = sprintf( wp_kses( __(
                'Individual CLNDR instances can be accessed directly from the <a href="%s">master stylesheet</a>. Their shortcode ID is applied <b>as a class</b> to the parent container, prepended by "clndr_".', 'event-clndr'
        ...
        <table class="widefat page fixed" width="100%" style="width:100%;max-width:900px;" cellpadding="3" cellspacing="3">
            <tr>
                <th class="manage-column" scope="col"><?php _e('Name','event-clndr') ?></th>
                <th class="manage-column" scope="col"><?php _e('Shortcode','event-clndr') ?></th>
            ...
            <?php
                ...
                foreach ( $instances as $instance )	{
                    ...
                    <tr class="<?php echo $class; ?>">
                        ...
                        <td><a href="<?php echo $wp_CLNDR_edit_url; ?>" class='edit'><?php _e('Edit','event-clndr'); ?></a></td>
                        <td><a href="<?php echo $wp_CLNDR_delete_url; ?>" class="delete" onclick="return confirm('<?php _e('Are you sure you want to delete this instance? Any active shortcodes will invalidate','event-clndr'); ?>')"><?php _e('Delete','event-clndr'); ?></a></td>
                    </tr>
                <?php
                }
            ?>
        ...
        ?><p><?php _e("There are no instances in the database",'event-clndr');?></p><?php
    }
}
                    ...
                    if ($verify["status"] == "success") {
                        ...
                        if ($action == "add") {
                            ...
                            $sql = $wpdb->prepare(
                                ...
                                $db_vars["title"],$db_vars["desc"],$db_vars["begin"],$db_vars["end"],$db_vars["time"],$db_vars["link"],$db_vars["link_target"],$event_id
                            );
                            $wpdb->get_results($sql);
                            ?>
...
function admin_CLNDR_EVENTS_display_events(){
    ...
    ?>
            ...
            <?php
                ...
                foreach ( $events as $event )	{
                    ...
                    <tr class="<?php echo $class; ?>">
                        ...
                        <td><a href="<?php echo $wp_CLNDR_edit_url; ?>" class='edit'><?php _e('Edit','event-clndr'); ?></a></td>
                        <td><a href="<?php echo $wp_CLNDR_delete_url; ?>" class="delete" onclick="return confirm('<?php _e('Are you sure you want to delete this event?','event-clndr'); ?>')"><?php _e('Delete','event-clndr'); ?></a></td>
                    </tr>
                <?php
                }
            ?>
        ...
        _e("There are no events in the database",'event-clndr');
    }
}
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="event_link_target" name="event_link_target" style="width:100%;max-width:320px;" <?php if ($wp_CLNDR_link == '') { echo "disabled"; } ?>>
                                <option value="_self" <?php if ($wp_CLNDR_link_target == "_self") { echo 'selected'; } ?>><?php _e("Same window/tab","event-clndr"); ?></option>
                                <option value="_blank" <?php if ($wp_CLNDR_link_target == "_blank") { echo 'selected'; } ?>><?php _e("New window/tab","event-clndr"); ?></option>
                                <option value="clndr" <?php if ($wp_CLNDR_link_target == "clndr") { echo 'selected'; } ?>><?php _e("Unique window/tab (open all links in this plugin in the same external window/tab)","event-clndr"); ?></option>
                            </select>
                        </td>
                    </tr>
                </table>
